...
- **Budget**: Workers Paid since 2026-09-22 (30s CPU / 10,000 subrequests per invocation, 50M D1 row writes/month, 1M KV writes/month); R2, Pages and KV stay within their free allowances; respect TBA API rate limits — hobby project economics
...
| **KV** (thin manifest layer only) | A handful of small, hot keys: `event:{key}:last-updated`, `event:{key}:live` flag, `data-version` | Historical: this thin-manifest design was originally chosen under the free tier's 100k reads/day and **1,000 writes/day** cap. KV writes are 1M/month on the paid plan since 2026-09-22, well clear of that old cap. Use it only for small pointer/flag values read very frequently and written rarely — not as the artifact store itself. Eventual consistency (~60s global propagation) is a non-issue for values updated once per minute, since it's below the propagation window's effect on freshness perception |
...
| **D1** (live per-team algorithm state, Worker-internal only) | R2's `get()` has no multi-key batch form, so one object per team costs one subrequest per team — a peak tick's ~21 team reads plus ~21 writes would have consumed 42 of the free plan's 50 subrequests per invocation before a single TBA poll (historical: the per-invocation limit is 10,000 on the paid plan since 2026-09-22, so this subrequest argument no longer forces the design). D1's `batch()` and a single `WHERE team_key IN (...)` collapse that to about two. R2 stays the primary artifact store, KV stays a thin manifest layer, and D1 is never reachable from the browser — this row narrows, not replaces, the "What NOT to Use" D1 row below. *(Narrowed by Phase 4 under `04-CONTEXT.md` D-13, 2026-08-22 — see that row's own updated "Why" column for the client-facing/Worker-internal split this narrowing draws.)* |
...
| GitHub-Actions/local Node pipeline for bulk compute | Doing all compute inside the Workers Cron Trigger | Only viable if you're certain every single computation — including hyperparameter tuning and simulations — will always fit within the per-invocation CPU budget (10ms on the free plan when this was written; 30s on Workers Paid since 2026-09-22). Given the failure log's "unidentifiable 4D model" and "no evaluation harness" history, don't bet the whole pipeline on a tight CPU budget you haven't measured against yet |

## What NOT to Use

| Avoid | Why | Use Instead |
|-------|-----|--------------|
| KV as the primary artifact store | Historical: before 2026-09-22 the free tier's write cap was 1,000/day; a cron job firing every 1 minute across a full ~10-hour event day is already ~600 writes on its own, and multiple concurrent events (regional season) would have blown past the cap fast. KV is 1M writes/month on the paid plan now, but the verdict is unchanged — KV is meant for small, hot values, not the bulk JSON payload store | R2 for artifacts; KV for a small manifest/pointer layer only |
...
| Cron Trigger doing full-season recompute or the 1000-run simulation | This is exactly the kind of "recompute-per-request" trap the failure log already names as a past failure mode, just moved from "per HTTP request" to "per cron tick" — full-season recomputes, hyperparameter tuning and the 1000-run simulation belong in the offline pipeline or the browser, never inside a cron tick, regardless of the invocation's CPU budget. Historical: this row originally argued from the free plan's 10ms CPU time, which would not have covered it. **The 10ms was never a flat per-invocation ceiling** — isolates carry built-in flexibility, and termination followed from hitting the limit *consistently*, not from one expensive tick. Verified against production 2026-08-29, under the free plan (Phase 7 debug session `worker-tick-exceeds-cpu-budget`): the same Worker version returned `ok` at `cpuTime:38`, then was killed pinned at `10` sixty seconds later, same code and same data. That discipline — budget for the *sustained* cost of the common path, never read a single healthy over-budget tick as evidence the budget is safe — is why bulk compute stays out of the cron tick regardless of how large the current ceiling is (30s CPU per invocation on Workers Paid since 2026-09-22) | Incremental per-match updates in the Worker; full recomputes in the offline Node pipeline; simulation client-side |
