...
| 16 | 07 | deviation | apps/worker/src/scheduled.ts |  | Deployed Worker (version 638da16c-d538-4551-b3a0-a2757a77061f, surfaced by plan 07-19 Task 3) observed exceeding the free-plan CPU budget on 100% of ticks captured (7/7 across two capture windows, several hours after the deploy): outcome:"exceededCpu", cpuTime pinned at 10, empty logs array on every tick -- meaning the tick's own console.log never ran. Directly contradicts the healthy ticks (4 then 3, all "ok":true) recorded immediately post-deploy on the SAME version. CORRECTED 2026-08-29 -- the original claim "No event was live in either observation" is DISPROVEN and was what misdirected the first investigation toward module init. The deployed live-windows manifest (generation 47d020a4-1a16-4331-bd70-ce2f468bf2d1) reported TWO events live: 2026azscor and 2026scsc, both PHANTOM `inferred` windows synthesised from start_date for zero-match offseason events. The operator judged liveness by "is a real competition happening"; the Worker judges it by the manifest, and the two disagreed. A 2026-08-29T21:55Z capture then caught a SURVIVING tick logging eventsConsidered:2 at cpuTime 38 ms -- proving the tick reaches the handler and runs the full live path, and that the empty logs array only ever proved non-completion, never non-entry. Root cause is an AND-gate: (A) per-tick Zod validation of all 1,581 live-windows, 1,542 of them permanently closed, costing 5-9 ms of the 10 ms budget on an idle tick; (B) the phantom windows keeping that tick out of its early exit. Fix committed 2026-08-29 (filter-at-read in liveWindows.ts, prune-at-build plus no-blind-windows in manifests.ts, regression tests for both); still open pending the deploy read-back that observes a real tick returning ok:true. Investigation: .planning/debug/worker-tick-exceeds-cpu-budget.md | open |  | 2026-08-29T20:41:00.000Z |  |
...
[
  ...
  },
  {
    ...
    "file": "apps/worker/src/scheduled.ts",
    "line": null,
    "description": "Deployed Worker (version 638da16c-d538-4551-b3a0-a2757a77061f, surfaced by plan 07-19 Task 3) observed exceeding the free-plan CPU budget on 100% of ticks captured (7/7 across two capture windows, several hours after the deploy): outcome:\"exceededCpu\", cpuTime pinned at 10, empty logs array on every tick -- meaning the tick's own console.log never ran. Directly contradicts the healthy ticks (4 then 3, all \"ok\":true) recorded immediately post-deploy on the SAME version. CORRECTED 2026-08-29 -- the original claim \"No event was live in either observation\" is DISPROVEN and was what misdirected the first investigation toward module init. The deployed live-windows manifest (generation 47d020a4-1a16-4331-bd70-ce2f468bf2d1) reported TWO events live: 2026azscor and 2026scsc, both PHANTOM `inferred` windows synthesised from start_date for zero-match offseason events. The operator judged liveness by \"is a real competition happening\"; the Worker judges it by the manifest, and the two disagreed. A 2026-08-29T21:55Z capture then caught a SURVIVING tick logging eventsConsidered:2 at cpuTime 38 ms -- proving the tick reaches the handler and runs the full live path, and that the empty logs array only ever proved non-completion, never non-entry. Root cause is an AND-gate: (A) per-tick Zod validation of all 1,581 live-windows, 1,542 of them permanently closed, costing 5-9 ms of the 10 ms budget on an idle tick; (B) the phantom windows keeping that tick out of its early exit. Fix committed 2026-08-29 (filter-at-read in liveWindows.ts, prune-at-build plus no-blind-windows in manifests.ts, regression tests for both); still open pending the deploy read-back that observes a real tick returning ok:true. Investigation: .planning/debug/worker-tick-exceeds-cpu-budget.md",
    "status": "open",
    "reason": "",
