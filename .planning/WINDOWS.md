...
|----|-------|------|------|------|-------------|--------|--------|-------------|-------------|
| 1 | 01 | stub | packages/harness/statbotics.ts |  | STATBOTICS_REFERENCE_FALLBACK per-season values (2022-2026) are unverified best-available estimates (~0.70-0.72), not individually sourced against Statbotics' own published figures — the live endpoint reproducibly 500s (reconfirmed 2026-08-13) and Statbotics' blog page renders numbers client-side from the same broken API, so this pipeline has no way to scrape verified numbers | fixed | Statbotics' API came back. Quick task 260904-4aa verified EPA against live api.statbotics.io (25/25 statistics pass, gated by --check against data/baselines/epa-vs-statbotics-2026-09.json) and, in doing so, found the deeper bug this row was really about: StatboticsYearResponseSchema parsed the RETIRED epa_acc field, so statboticsReference had been silently falling back to these unverified ~0.70-0.72 stubs on EVERY call even after the outage ended. Real published figures are 0.76-0.80, read from metrics.win_prob.season.{acc,mse}. The fallback constants are no longer the load-bearing source. Verdict in docs/models/epa-vs-statbotics.md. | 2026-08-13T16:13:38.414Z | 2026-09-06T00:00:00.000Z |
| 2 | 02 | deviation | packages/harness/statbotics.ts |  | SC-2 (Statbotics per-team numeric tolerance) recorded blocked-on-external-dependency per D-14 -- api.statbotics.io/v3/year/{year} reproducibly returns HTTP 500, re-confirmed live 2026-08-14; EPA correctness rests on synthetic-fixture tests and walk-forward structural proofs instead | fixed | SC-2 CLOSED by quick task 260904-4aa (2026-09-04). api.statbotics.io is reachable again; a tested, re-runnable comparison (scripts/epaVsStatbotics.ts + packages/harness/epaStatboticsCompare.ts) now checks EPA against Statbotics' epa.total_points with fouls excluded for comparability, gated on a committed tolerance baseline. PROJECT.md's SC-2 row was flipped from blocked to Held by quick task 40 (dbd6c19f). EPA correctness no longer rests on synthetic fixtures alone. | 2026-08-14T04:31:03.405Z | 2026-09-06T00:00:00.000Z |
...
| 6 | 04 | stub | apps/worker/src/scheduled.ts |  | runGlobalRebuild's incremental teams/{year} merge updates metrics/matchCount but NOT the win/loss/tie record field (would need per-team match outcomes threaded through touchedTeamsByAlgorithm, which the plan's time budget did not extend to) -- stays accurate only as of the last offline pnpm publish:seasons run until a future plan extends this | waived | Accepted as a known v1.0 limitation at milestone close, and recorded in STATE.md's Deferred Items table rather than left as a silent open row. Each of these five was written as a deliberate scope boundary at the time -- their own descriptions already say 'deferred to a future plan' or 'not fixed in this plan (architectural)' -- so this waive records an acceptance that already happened, it does not make a new one. None affects the offline publish path that produces every artifact the site actually serves; all five concern the live incremental Worker only. | 2026-08-22T18:03:00.607Z | 2026-09-06T00:00:00.000Z |
| 7 | 04 | stub | apps/worker/src/scheduled.ts |  | The online path never rebuilds events/{year} at all (only teams/{year} via runGlobalRebuild) -- the events list stays accurate only as of the last offline pnpm publish:seasons run; extending the incremental-merge mechanism to events/{year} is deferred to a future plan | waived | Accepted as a known v1.0 limitation at milestone close, and recorded in STATE.md's Deferred Items table rather than left as a silent open row. Each of these five was written as a deliberate scope boundary at the time -- their own descriptions already say 'deferred to a future plan' or 'not fixed in this plan (architectural)' -- so this waive records an acceptance that already happened, it does not make a new one. None affects the offline publish path that produces every artifact the site actually serves; all five concern the live incremental Worker only. | 2026-08-22T18:03:06.054Z | 2026-09-06T00:00:00.000Z |
| 8 | 04 | deviation | apps/worker/src/scheduled.ts |  | Phase B (artifact writes) is deliberately best-effort: a failure there does not change an event's 'advanced' outcome (state has genuinely advanced correctly), but a skipped artifact stays one tick stale until that team's next match at that event -- no future trigger re-attempts a partially-completed Phase B on its own | waived | Accepted as a known v1.0 limitation at milestone close, and recorded in STATE.md's Deferred Items table rather than left as a silent open row. Each of these five was written as a deliberate scope boundary at the time -- their own descriptions already say 'deferred to a future plan' or 'not fixed in this plan (architectural)' -- so this waive records an acceptance that already happened, it does not make a new one. None affects the offline publish path that produces every artifact the site actually serves; all five concern the live incremental Worker only. | 2026-08-22T18:03:11.610Z | 2026-09-06T00:00:00.000Z |
| 9 | 04 | deviation | docs/publish-budget.md |  | 3 published algorithms folded together for a single ordinary match exceed the deployed Worker's real per-tick subrequest budget (estimated cost 50 vs usable ~41); confirmed by repeated live observation, not fixed in this plan (Rule 4 -- architectural) | waived | Accepted as a known v1.0 limitation at milestone close, and recorded in STATE.md's Deferred Items table rather than left as a silent open row. Each of these five was written as a deliberate scope boundary at the time -- their own descriptions already say 'deferred to a future plan' or 'not fixed in this plan (architectural)' -- so this waive records an acceptance that already happened, it does not make a new one. None affects the offline publish path that produces every artifact the site actually serves; all five concern the live incremental Worker only. | 2026-08-23T03:17:45.065Z | 2026-09-06T00:00:00.000Z |
| 10 | 04 | unrun-verify | apps/worker/src/scheduled.ts |  | epa/sigma1 solo deployed-Worker freshness runs (plan 04-07 Task 2) never folded a single match within the poll window and were not diagnosed to root cause within the plan's session; opr's identical rig succeeded cleanly (6/6, digestMatch=true). Not reproduced in an isolated local call to the same algorithm code with the same real data. The CI-runnable offline equivalence test (scheduled.replay.test.ts) independently proves equivalence for all three algorithms and is unaffected. | waived | Accepted as a known v1.0 limitation at milestone close, and recorded in STATE.md's Deferred Items table rather than left as a silent open row. Each of these five was written as a deliberate scope boundary at the time -- their own descriptions already say 'deferred to a future plan' or 'not fixed in this plan (architectural)' -- so this waive records an acceptance that already happened, it does not make a new one. None affects the offline publish path that produces every artifact the site actually serves; all five concern the live incremental Worker only. | 2026-08-23T03:17:53.122Z | 2026-09-06T00:00:00.000Z |
| 11 | 06.1 | unmet-truth | docs/publish-budget.md |  | teams/{year} page kind measured max, re-measured again by exclude-whole-alliance-dq-zero-scores.md's 2026-08-30 post-fix republish (generation 1c11cdd8-720d-479e-a737-fad94c4105a9): now 3,704,776 bytes (was 3,705,194 per exclude-offseason-demo-teams.md's 2026-08-29 republish, was 3,732,955 per 07-17's D-18 republish, was 3,577,069 before that) -- a noise-level -418-byte movement (the DQ fix changes computed VALUES via dropped alliance-observations, not published fields), STILL over committed budgetMaxBytes (3,500,000) -- neither this fix nor the demo-team exclusion before it were ever expected to clear this ceiling (explicit non-goal of both); ceiling still deliberately not raised; payloadBudget.test.ts left red pending a developer decision | fixed | CLEARED ON MERIT by quick task 260902-pbe (b75b49cf), which encoded the teams artifact's metrics positionally with the key list in the preamble: 3,704,776 -> 1,489,187 bytes raw, a 59.8% reduction that puts the page kind comfortably under its unchanged 3,500,000 budgetMaxBytes. The ceiling was never raised. Verified 2026-09-06: payloadBudget.test.ts passes 11/11. | 2026-08-27T00:37:40.533Z | 2026-09-06T00:00:00.000Z |
| 15 | 07 | unmet-truth | docs/publish-budget.md |  | team/{teamKey}/{year} page kind still crosses BOTH its committed budgetMaxBytes (375,000) and payloadBudget.test.ts's separate absolute structural ceiling (TEAM_PAGE_ABSOLUTE_MAX_BYTES = 600,000), re-measured against exclude-whole-alliance-dq-zero-scores.md's 2026-08-30 post-fix republish (generation 1c11cdd8-720d-479e-a737-fad94c4105a9): max now 675,956 bytes (80.3% over budget, 12.7% over the absolute ceiling), a noise-level +13-byte movement from 675,943 bytes measured against exclude-offseason-demo-teams.md's 2026-08-29 republish (was 821,938 bytes, 119% over budget, 37% over the absolute ceiling, per 07-17's D-18 republish) -- same max-holding key (v1/team/frc3538/2024/vpr@..., a real team, 234 played matches in the 2024 season) both times. Neither the DQ fix nor the demo-team exclusion before it were ever expected to clear either ceiling (explicit non-goal of both) and neither did -- both stay open for their own, separate resolution. Neither ceiling raised | fixed | Resolved by an explicit, user-authorized ceiling raise (fast-260904, 4283559e): team/{teamKey}/{year} budgetMaxBytes 375,000 -> 400,000, taken as a deliberate decision on a real measured overage rather than a silent accommodation. TEAM_PAGE_ABSOLUTE_MAX_BYTES (600,000) was left untouched. Verified 2026-09-06: payloadBudget.test.ts passes 11/11. | 2026-08-28T18:55:00.000Z | 2026-09-06T00:00:00.000Z |
...
| 17 | 07 | deviation | apps/web/e2e/event-live-artifact.spec.ts |  | 07-20's alliance-uncertainty identity test (sigma_alliance from published as-of-event metrics.total.spread vs sigma_match from a match's own redScoreVarianceOwn/blueScoreVarianceOwn) measured FALSE on real data (99/99 pairs across 2024new/2023cur/2024casf/2025flta exceeded a derived 0.02 tolerance, diffs 0.03-2.04 sigma units). Root-caused as a spec bug, not a data or model defect: the two quantities are measured at different points in the walk-forward (as-of-event vs as-of-that-match); a direct developer check (280 alliance-pairs, 2024new) confirmed the signature -- mean gap 1.130 in the first half of the event vs 0.373 in the second half, a ~3x collapse consistent with variance shrinking as more matches are observed. The plan's own tolerance derivation was independently found to be wrong in the safe direction (ROUNDING_RULE.variance is 4 decimals, not the 2 the plan assumed), so that correction only strengthened the finding rather than explaining it away. | resolved | Test corrected to assert the provable monotone-narrowing relationship (mean absolute gap in the event's second half is smaller than in its first half) instead of near-equality. The TRUE identity (same-instant per-team spread vs. that instant's alliance variance) remains untestable until the pipeline publishes as-of-match (not as-of-event) team metrics -- routed to .planning/todos/pending/publish-as-of-match-team-metrics.md as an actionable follow-up. | 2026-08-29T00:00:00.000Z | 2026-08-29T00:00:00.000Z |
...
[
  {
    ...
    "status": "fixed",
    "reason": "Statbotics' API came back. Quick task 260904-4aa verified EPA against live api.statbotics.io (25/25 statistics pass, gated by --check against data/baselines/epa-vs-statbotics-2026-09.json) and, in doing so, found the deeper bug this row was really about: StatboticsYearResponseSchema parsed the RETIRED epa_acc field, so statboticsReference had been silently falling back to these unverified ~0.70-0.72 stubs on EVERY call even after the outage ended. Real published figures are 0.76-0.80, read from metrics.win_prob.season.{acc,mse}. The fallback constants are no longer the load-bearing source. Verdict in docs/models/epa-vs-statbotics.md.",
  ...
  },
  {
    ...
    "status": "fixed",
    "reason": "SC-2 CLOSED by quick task 260904-4aa (2026-09-04). api.statbotics.io is reachable again; a tested, re-runnable comparison (scripts/epaVsStatbotics.ts + packages/harness/epaStatboticsCompare.ts) now checks EPA against Statbotics' epa.total_points with fouls excluded for comparability, gated on a committed tolerance baseline. PROJECT.md's SC-2 row was flipped from blocked to Held by quick task 40 (dbd6c19f). EPA correctness no longer rests on synthetic fixtures alone.",
  ...
  },
  {
    ...
    "status": "waived",
    "reason": "Accepted as a known v1.0 limitation at milestone close, and recorded in STATE.md's Deferred Items table rather than left as a silent open row. Each of these five was written as a deliberate scope boundary at the time -- their own descriptions already say 'deferred to a future plan' or 'not fixed in this plan (architectural)' -- so this waive records an acceptance that already happened, it does not make a new one. None affects the offline publish path that produces every artifact the site actually serves; all five concern the live incremental Worker only.",
  ...
  },
  {
    ...
    "status": "fixed",
    "reason": "CLEARED ON MERIT by quick task 260902-pbe (b75b49cf), which encoded the teams artifact's metrics positionally with the key list in the preamble: 3,704,776 -> 1,489,187 bytes raw, a 59.8% reduction that puts the page kind comfortably under its unchanged 3,500,000 budgetMaxBytes. The ceiling was never raised. Verified 2026-09-06: payloadBudget.test.ts passes 11/11.",
  ...
  },
  {
    ...
    "status": "resolved",
    "reason": "Ledger hygiene correction (no code change): this entry's own description already documents the fix as landed and verified -- extractRp requires Number.isInteger and degrades to null, the toIntegerRpOrNull defence-in-depth guard is in place, the two events were re-ingested, and non-integer rows in data/corpus.sqlite went from 30 to 0, with regression tests in normalize.test.ts and publish.test.ts. Status was left open in error; corrected to match the text.",
  ...
  },
  {
    ...
    "file": "docs/publish-budget.md",
    "line": null,
    "description": "team/{teamKey}/{year} page kind still crosses BOTH its committed budgetMaxBytes (375,000) and payloadBudget.test.ts's separate absolute structural ceiling (TEAM_PAGE_ABSOLUTE_MAX_BYTES = 600,000), re-measured against exclude-whole-alliance-dq-zero-scores.md's 2026-08-30 post-fix republish (generation 1c11cdd8-720d-479e-a737-fad94c4105a9): max now 675,956 bytes (80.3% over budget, 12.7% over the absolute ceiling), a noise-level +13-byte movement from 675,943 bytes measured against exclude-offseason-demo-teams.md's 2026-08-29 republish (was 821,938 bytes, 119% over budget, 37% over the absolute ceiling, per 07-17's D-18 republish) -- same max-holding key (v1/team/frc3538/2024/vpr@..., a real team, 234 played matches in the 2024 season) both times. Neither the DQ fix nor the demo-team exclusion before it were ever expected to clear either ceiling (explicit non-goal of both) and neither did -- both stay open for their own, separate resolution. Neither ceiling raised",
    "status": "fixed",
    "reason": "Resolved by an explicit, user-authorized ceiling raise (fast-260904, 4283559e): team/{teamKey}/{year} budgetMaxBytes 375,000 -> 400,000, taken as a deliberate decision on a real measured overage rather than a silent accommodation. TEAM_PAGE_ABSOLUTE_MAX_BYTES (600,000) was left untouched. Verified 2026-09-06: payloadBudget.test.ts passes 11/11.",
  ...
  },
  {
    ...
    "kind": "deviation",
    "phase": "07",
    "file": "apps/web/e2e/event-live-artifact.spec.ts",
    "line": null,
    "description": "07-20's alliance-uncertainty identity test (sigma_alliance from published as-of-event metrics.total.spread vs sigma_match from a match's own redScoreVarianceOwn/blueScoreVarianceOwn) measured FALSE on real data (99/99 pairs across 2024new/2023cur/2024casf/2025flta exceeded a derived 0.02 tolerance, diffs 0.03-2.04 sigma units). Root-caused as a spec bug, not a data or model defect: the two quantities are measured at different points in the walk-forward (as-of-event vs as-of-that-match); a direct developer check (280 alliance-pairs, 2024new) confirmed the signature -- mean gap 1.130 in the first half of the event vs 0.373 in the second half, a ~3x collapse consistent with variance shrinking as more matches are observed. The plan's own tolerance derivation was independently found to be wrong in the safe direction (ROUNDING_RULE.variance is 4 decimals, not the 2 the plan assumed), so that correction only strengthened the finding rather than explaining it away.",
    "status": "resolved",
    "reason": "Test corrected to assert the provable monotone-narrowing relationship (mean absolute gap in the event's second half is smaller than in its first half) instead of near-equality. The TRUE identity (same-instant per-team spread vs. that instant's alliance variance) remains untestable until the pipeline publishes as-of-match (not as-of-event) team metrics -- routed to .planning/todos/pending/publish-as-of-match-team-metrics.md as an actionable follow-up.",
