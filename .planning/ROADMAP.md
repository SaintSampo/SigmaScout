...
- [x] 06-04-PLAN.md — Publisher wiring: event-name fix, own variance, actual RP, scheduled rows, percentiles, activeYears, robot image
- [x] 06-05-PLAN.md — Metric-history chart tab (Recharts, measured dynamic import) + search box and Teams table integration

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 06-06-PLAN.md — The single full republish and the payload-budget re-measure

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 06-07-PLAN.md — Season header, rarity tiers, team-scoped year dropdown
- [x] 06-08-PLAN.md — Event sections and the predicted-vs-actual match table on one shared axis
...
- [x] 06.1-04-PLAN.md — Full 2022–2026 rankings ingest and the rankings endpoint's COVERAGE.md capability matrix
...
- [x] 06.1-06-PLAN.md — UI: real bonus-dot states, per-event rarity tiers, and the visible statement of what those tiers rank against
...
**Waves 6-10** *(each blocked on the previous; client tabs)*
  ...
  - `07-14` — Alliances tab (EVNT-05, D-15..D-17)
...
| 9. Analytic Ranking Points & Browser-Side Simulation | 10/10 | In Progress|  |
...
7. Rank simulation: draw each match outcome **once** and then bonuses per alliance, replacing today's independent red/blue marginal draws where both alliances can win the same draw. Monte Carlo is **kept** for the rank step -- it approximates a joint with no tractable closed form and captures shared-match coupling an analytic form would discard. Confirmed by the developer during discussion ("I dont mind MC for ranks prediction").
8. Pre-schedule prediction redesigned as a **laddered spike** (reshaped by discussion, 2026-09-11 -- see `09-CONTEXT.md` D-16 through D-19). The 20 synthetic schedules are themselves a Monte Carlo approximation of an expectation over schedule randomness -- the same insight as the RP pmf, one level up -- so concrete schedules may not be needed at all. Measure at each rung, stop at the first that holds:
   1. **Field-averaged analytic**, no schedule generation: build each team's per-match RP pmf from its own belief plus field-level summary statistics (mean and variance of per-team contributions across the roster), then convolve N copies for the season total. Target artifact ~2-3 KB versus ~265 KB today, with schedule generation deleted entirely. Ship if rank bands match the current baked output on real events.
   2. **Self-generated random schedules** (~20 KB): test whether they reproduce cheesy-arena balanced-grid results, answering whether the balanced structure matters to final rank bands or is merely tradition.
   3. **Licensed templates** as today.
   Independent of the rung: re-key/regenerate the sidecars (every one in R2 belongs to retired `vpr`) and re-enable presim generation (currently off via `--presim-from-season 9999`), or the pre-schedule stop stays dark. Schedule-template licensing is now **conditional, not blocking** -- it only needs answering if rung 1 fails and rung 2 is chosen.
...
- Schedule-template redistribution licensing -- now conditional on the pre-schedule ladder reaching rung 2 (see deliverable 8); it evaporates if rung 1 holds.
...
- [x] 10-03-PLAN.md — district artifact and manifest contract, shared `applyDistrictRankings`, `publish:live-windows`
...
- [x] 10-07-PLAN.md — the web tab: Worker triad and the Road to District Champs ledger
