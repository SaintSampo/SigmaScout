...
winner: "B — shared 1..N axis with interpolated (continuous) 10th–90th band edges"
...
## Winner — B, interpolated band edges (user, 2026-08-30)

Jacob: *"005 B is best."* The shipped rank plot keeps the shared `1..N` axis, the per-row histogram,
and the median tick, and computes the 10th–90th band's start and stop as **continuous** rank
coordinates rather than reading the integer `p10`/`p90` fields.

**The method, so the implementation does not reinvent it.** Treat rank `r` as occupying
`[r-0.5, r+0.5)` with its probability mass spread uniformly inside that interval. The CDF becomes
piecewise-linear and the quantile continuous. This matches R's default (type-7) quantile behaviour on
binned data — it is a standard estimator, not a bespoke smoothing. `continuousQuantile()` in
`index.html` is the reference implementation.

**It is bounded by construction**, which is why it does not reintroduce the problem that ruled out
mean ± SD: an edge can never fall below `0.5` or above `N+0.5`, so a band never implies a rank that
cannot exist. (Mean ± SD does: mean 3.0 with SD 4.0 spans −1.0 to 7.0.)

**Three defects it fixes, measured on real data, not argued:**

| | Snapped | Interpolated |
|---|---|---|
| 3467 @ 2023nhgrs (locked at rank 1) | `1–1`, width **0 — invisible** | `0.60–1.40`, width 0.80 |
| 95 @ 2023nhgrs | `2–3`, width 1 | `1.65–3.20`, width 1.55 |
| 4564 @ 2023nhgrs | `2–3`, width 1 | `1.80–3.49`, width 1.69 |
| 386 @ 2025flta (locked) | `1–1`, width **0 — invisible** | `0.60–1.42`, width 0.82 |

1. **Locked teams stop vanishing.** 2 of 39 rows at `2023nhgrs` and 3 of 42 at `2025flta` draw a
   zero-width, invisible band under integer snapping.
2. **Rows that were falsely identical separate.** Teams 95 and 4564 both render as `2–3` width 1
   despite genuinely different distributions.
3. **Up to a full rank of quantization error at each edge is removed.**

**Binding constraints carried into implementation.** The band is still NOT a `±` quantity — Phase 7
D-01 reserves that glyph for one standard deviation of full predictive variance, and this is a
percentile range. Label it explicitly (`10th–90th: 1.7–3.2`), one decimal place, never `±`. The
x-scale must map continuous rank coordinates through the SAME single `x()` function the histogram and
median tick use — one computed source, per `chart-craft.md`'s coupled-geometry rule.

**This supersedes the UI-SPEC.** `08-UI-SPEC.md`'s rank-plot contract describes the integer-snapped
band. It must be updated via `/gsd-ui-phase 8` → Update before planning, or the spec and the decision
will disagree.
