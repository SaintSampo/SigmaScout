...
| offseason events in ingest scope | INTEGRATE | TEAM-04's "attended... event" wording is not scoped to in-season, and offseason events are 553 of the corpus's 1,581 (35%); TBA computes these rankings from official match results — see note [4] |
...
| `GET /event/{event_key}/oprs` | OPT-OUT | this project computes its own event-scoped OPR (`packages/core/algorithms/opr.ts`); ingesting TBA's differently-computed OPR alongside it is the two-implementations-drift anti-pattern — see note [5] |
...
| `GET /event/{event_key}/insights` | OPT-OUT | TBA's per-event insights are a season-specific aggregate statistics blob (season-varying vocabulary, RESEARCH.md Pitfall 3) with no display slot in the current UI; not requested by TEAM-04 |
...
| `rankings` array length (as the standing's total, "of M") | INTEGRATE | `response.rankings.length` is the true reported pool size, stored as `event_rankings.total_teams` per row, independent of how many rows this corpus could resolve to a known team key — see note [1] |
...
| `qual_average` | OPT-OUT | observed `null` in every 2022-2026 live sample this session (RESEARCH.md Assumption A1); TBA appears to have moved the concept into `sort_orders` — ingesting a never-populated field adds no value |
| `sort_orders` | OPT-OUT | this phase's success criterion needs only `rank` and `rankings.length`; the array's per-index meaning is season-specific and undocumented without cross-referencing `sort_order_info` (see that row) |
| `extra_stats` | OPT-OUT | same reasoning as `sort_orders` — undisplayed, season-specific, not needed for "Rank N of M" |
| `sort_order_info` (per-season sort-order column names) | OPT-OUT | the names genuinely differ by season, confirmed live across 2022/2024/2026; "Rank N of M" needs none of them, and a per-season name table is scope creep this phase does not ask for — see note [6] |
...
**[4] Offseason events in ingest scope.** TEAM-04's "attended... event" wording is not scoped to
in-season only, and this project's own corpus shows offseason events are 553 of the corpus's 1,581
total events (35%) — excluding them would leave over a third of every attended-event section with no
standing at all. TBA computes these rankings itself from official match results, not from the
self-reported `score_breakdown` JSON that Phase 3's RP fold deliberately excludes offseason events
from — a materially different trust boundary. The request-count cost delta is negligible (1,581 vs
1,028 events, roughly one extra minute of throttled requests at 100ms/request). Settles
RESEARCH.md Open Question 1 as a decision, per PD-01.

**[5] `GET /event/{event_key}/oprs`.** This project computes its own event-scoped OPR
(`packages/core/algorithms/opr.ts`, `solveEventOpr`) as one of its three v1 algorithms; ingesting
TBA's own differently-computed OPR alongside it is the two-implementations-that-drift anti-pattern
`.planning/research/ARCHITECTURE.md` names — a reader comparing "TBA's OPR" against "SigmaScout's
OPR" for the same event would see two numbers with no way to tell which is authoritative.

**[6] `sort_order_info`.** The names genuinely differ by season — confirmed live: 2022 uses four
names (Ranking Score, Avg Match, Avg Hangar, Avg Taxi + Auto Cargo), 2024 uses five (adds Avg
Coop), 2026 uses three different names again (Avg Auto Fuel, Avg Tower, ...). "Rank N of M" needs
none of them; building a per-season name table here would be scope creep this phase's own success
criterion does not ask for (RESEARCH.md Pitfall 3).
