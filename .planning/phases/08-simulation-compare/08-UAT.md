...
result: passed — swiping, scrolling, flicking and tapping all good (iPhone + Android)
...
result: passed — several genuinely bimodal rows found at 2023cur; rows read as individual distributions, pinned columns hold, no ± glyph
...
result: passed — page scrolled smoothly through the run on real hardware; no hitch or stall (SC-2 non-blocking confirmed off-emulator)
...
result: passed — strip drags independently, Simulation reachable, no wrapped labels; D-04 plain-disabled state reads as intentional rather than broken
...
result: passed — accuracy table pans independently, no jump on compLevel switch, calibration chart legible with sparse dots still visible
