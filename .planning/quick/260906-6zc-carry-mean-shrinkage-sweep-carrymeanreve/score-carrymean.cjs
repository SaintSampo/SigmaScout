...
/**
 * Quick task 260906-6zc, Task 1.
 *
 * Joins N+2 series on matchKey — baseline (vpr, reused from
 * reports/rpnoise-baseline-260905), epa (algorithmId==='epa' rows only, from
 * reports/autopsy-260905), and any number of carry-mean-reversion arms from
 * reports/carrymean-{arm}-260906 — scores accuracy/Brier over the full scored
 ...
 *
 * Usage: node score-carrymean.cjs [--arms r25,r40,r60]
 */
...
const ARM_DIR = (name) => path.join(REPO, "reports", `carrymean-${name}-260906`);
...
async function main() {
  ...
  console.log(`score-carrymean.cjs — arms=${arms.length > 0 ? arms.join(",") : "none"}`);
