 ...
 *
 * Usage: node score-gaincap.cjs [--arms g60,g45,g30]
 */
...
const ARM_DIR = (name) => path.join(REPO, "reports", `gaincap-${name}-260906`);
...
async function main() {
  ...
  console.log(`score-gaincap.cjs — arms=${arms.length > 0 ? arms.join(",") : "none"}`);
