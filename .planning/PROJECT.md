...
- [ ] Team page: robot image, TBA link, season stats, per-event sections with match-level predictions (winner, confidence, scores, RP ± variance) vs actuals, metric-history plot — *Phase 6 built every listed surface; Phase 06.1 backed the per-match and per-event detail with real published data (per-bonus RP, per-event standing, per-event rarity tiers). Held in Active because Phase 6 gap G-06-2 (layout quality at desktop and phone widths, major) is still open — the capabilities exist, the layout pass does not.*
