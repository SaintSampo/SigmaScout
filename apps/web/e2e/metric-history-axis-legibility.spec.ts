 ...
 *
 * `frc4788`/2026 is the reproduction case named in the gap report — its
 * published `total` ran deeply negative (a modelling/cold-start artifact,
 * NOT fixed here — see `.planning/todos/pending/exclude-whole-alliance-dq-zero-scores.md`).
 * `frc254`/2026 is the normal-range control: this fix must not regress
 * the common case (short labels, no clipping risk) while fixing the extreme
 * one.
 */
const EXTREME_TEAM_ROUTE = "/team/4788?year=2026&algorithm=spr&tab=history";
const NORMAL_TEAM_ROUTE = "/team/254?year=2026&algorithm=spr&tab=history";
...
test("extreme negative team (frc4788, 2026, spr): no float-noise tick labels, none clipped at the plot's left edge", async ({ page }) => {
...
test("normal team (frc254, 2026, spr): unaffected by the extreme-value fix — no clipping, no regression", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 900 });
