...
/**
 * `/compare` redirect: the accuracy-comparison page moved to
 * `/methodology/compare`, but `/compare` has been shareable — a public
 * site must not break already-shared links. This route's only job is to
 * redirect to the moved page, carrying the current search params forward
 * unchanged.
 *
 * The redirect target is a single fixed literal path, never a value read
 * from the URL — no open-redirect and no redirect loop is constructible
 * here.
 *
