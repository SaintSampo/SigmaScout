/**
 * The home page: the site argues its case before anything else. The hero
 * states the claim, the podium shows pooled 2024 to 2026 winner accuracy
 * for all three published algorithms — computed at run time from the same
 * `v1/compare/{year}.json` artifacts the Compare page reads, never a
 * hand-typed number — and three CTAs route into the tool.
 *
 * Fast-load discipline: three small compare artifacts fetch in parallel
 * behind the CDN; the hero and CTAs render immediately, the podium area
 * holds a fixed-height skeleton while pending and simply hides on error
 * (the front door never shows a page-level failure for a decorative proof
 * block — the Compare page is the canonical home of these numbers).
 */
...
export function Podium() {
  ...
  // `pooledAccuracyPodium` throws by design on a malformed input — a
  // missing `combined` slice, a zero pooled `scoredCount`. Guarding only
  // `failed` above does not catch a well-formed HTTP 200 whose combined
  // slice is absent for one algorithm.
  //
  // The guard belongs at THIS call site rather than in the helper or an
  // error boundary: Compare wants the loud failure and its tests assert it,
  // only the front door wants the block hidden, and only the caller knows
  // which of the two it is. An error boundary would be a blunter instrument
  // in the opposite direction: it would swallow every render bug in its
  // subtree under the same "no podium" outcome, converting real breakage
  // into invisible absence.
  let podium: PodiumEntry[];
  try {
    // `useQueries` preserves input order, so `PODIUM_SEASONS[index]` is
    // exactly the year that query fetched — pairing them here (rather than
    // re-deriving a year from the fetched data, which the artifact does not
    // carry) is what lets `pooledAccuracyPodium` assert each slice against
    // the season its own caller asked for.
  ...
  // The caption's count is NOT guaranteed to be the same across all three
  // algorithms: `pooledAccuracyPodium` accumulates each algorithm's scored
  // count independently (a season with a null `winnerAccuracy` for one
  // algorithm is skipped for THAT algorithm alone, per `homePodium.ts`), so
  // "each" is not a safe claim in general even though it happens to be true
  // of every currently published artifact. Collect the distinct counts and
  // only claim "each" when there really is exactly one; otherwise render
...
function HomePage() {
  return (
    <main className="mx-auto flex w-full max-w-4xl flex-col items-center gap-[var(--spacing-2xl)] p-[var(--spacing-lg)] pt-[var(--spacing-2xl)] text-center">
      ...
      {/* A search bar ON the page, not only in the ribbon ("find your team"
          is still the most common arrival intent). `className="w-full"`
          fills this centered container. */}
