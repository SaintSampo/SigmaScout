...
describe("fetchTeamArtifact", () => {
  ...
  it("parses a well-formed fixture and returns a typed result", async () => {
    ...
    const result = await fetchTeamArtifact({ teamKey: "frc1114", year: 2024, algorithmId: "bpr", version: "2.0.0+tuned-2026-08" });
  ...
  it("raises ArtifactFetchError with the status on a 404", async () => {
    ...
    let caught: unknown;
    try {
      await fetchTeamArtifact({ teamKey: "frc1114", year: 2024, algorithmId: "bpr", version: "2.0.0+tuned-2026-08" });
    } catch (err) {
      caught = err;
    }
  ...
  it("raises ArtifactValidationError (not a bare throw) when a required field is missing", async () => {
    ...
    await expect(fetchTeamArtifact({ teamKey: "frc1114", year: 2024, algorithmId: "bpr", version: "2.0.0+tuned-2026-08" })).rejects.toBeInstanceOf(
  ...
  it("requests the exact key-built URL, proving artifactKey()'s team branch and the origin module are wired together", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("https://data.sigmascout.org/v1/team/frc1114/2024/bpr@2.0.0+tuned-2026-08.json");
  });
});
