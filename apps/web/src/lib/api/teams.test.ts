...
describe("fetchTeamsArtifact", () => {
  ...
  it("parses a well-formed fixture and returns typed rows", async () => {
    ...
    const result = await fetchTeamsArtifact({ year: 2024, algorithmId: "bpr", version: "2.0.0+tuned-2026-08" });
  ...
  it("raises ArtifactFetchError with the status on a 404", async () => {
    ...
    let caught: unknown;
    try {
      await fetchTeamsArtifact({ year: 2024, algorithmId: "bpr", version: "2.0.0+tuned-2026-08" });
    } catch (err) {
      caught = err;
    }
  ...
  it("raises ArtifactValidationError (not a bare throw) when a required field is missing", async () => {
    ...
    await expect(fetchTeamsArtifact({ year: 2024, algorithmId: "bpr", version: "2.0.0+tuned-2026-08" })).rejects.toBeInstanceOf(ArtifactValidationError);
  });
  ...
  it("requests the exact key-built URL, proving artifactKey() and the origin module are wired together", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("https://data.sigmascout.org/v1/teams/2024/bpr@2.0.0+tuned-2026-08.json");
  });
});
