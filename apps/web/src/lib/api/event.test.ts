...
describe("fetchEventArtifact", () => {
  ...
  it("parses a well-formed fixture and returns a typed result", async () => {
    ...
    const result = await fetchEventArtifact({ eventKey: "2024casf", algorithmId: "bpr", version: "2.0.0+tuned-2026-08" });
  ...
  it("raises ArtifactFetchError with the status on a 404, deriving year from the event key", async () => {
    ...
    let caught: unknown;
    try {
      await fetchEventArtifact({ eventKey: "2024casf", algorithmId: "bpr", version: "2.0.0+tuned-2026-08" });
    } catch (err) {
      caught = err;
    }
  ...
  it("raises ArtifactValidationError (not a bare throw) when the required teams key is missing", async () => {
    ...
    await expect(
      fetchEventArtifact({ eventKey: "2024casf", algorithmId: "bpr", version: "2.0.0+tuned-2026-08" }),
  ...
  it("requests the exact key-built URL, proving artifactKey()'s event branch and the origin module are wired together", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("https://data.sigmascout.org/v1/event/2024casf/bpr@2.0.0+tuned-2026-08.json");
  });
});
