...
describe("pooledAccuracyPodium", () => {
  it("pools as a scoredCount-weighted mean, never a mean of means", () => {
    const a = artifact(2024, { spr: { acc: 0.8, n: 100 }, epa: { acc: 0.7, n: 100 }, opr: { acc: 0.7, n: 100 } });
    const b = artifact(2025, { spr: { acc: 0.6, n: 900 }, epa: { acc: 0.7, n: 900 }, opr: { acc: 0.75, n: 900 } });
    const podium = pooledAccuracyPodium([seasoned(2024, a), seasoned(2025, b)]);
    const premier = podium.find((p) => p.algorithmId === "spr")!;
    // Weighted: (0.8*100 + 0.6*900) / 1000 = 0.62 — a mean-of-means would say 0.7.
    expect(premier.accuracy).toBeCloseTo(0.62, 10);
    expect(premier.scoredCount).toBe(1000);
  });

  it("sorts best-first — the podium order", () => {
    const a = artifact(2024, { spr: { acc: 0.76, n: 10 }, epa: { acc: 0.72, n: 10 }, opr: { acc: 0.73, n: 10 } });
    expect(pooledAccuracyPodium([seasoned(2024, a)]).map((p) => p.algorithmId)).toEqual(["spr", "opr", "epa"]);
  });

  it("a null-accuracy slice contributes nothing rather than poisoning the pool", () => {
    const a = artifact(2024, { spr: { acc: null, n: 50 }, epa: { acc: 0.7, n: 50 }, opr: { acc: 0.7, n: 50 } });
    const b = artifact(2025, { spr: { acc: 0.8, n: 100 }, epa: { acc: 0.7, n: 100 }, opr: { acc: 0.7, n: 100 } });
    const premier = pooledAccuracyPodium([seasoned(2024, a), seasoned(2025, b)]).find((p) => p.algorithmId === "spr")!;
    expect(premier.accuracy).toBeCloseTo(0.8, 10);
    expect(premier.scoredCount).toBe(100);
  });
  ...
  it("throws, naming the season, when an artifact carries only OTHER seasons' slices", () => {
    const wrongSeason = artifact(2023, { spr: { acc: 0.8, n: 100 }, epa: { acc: 0.7, n: 100 }, opr: { acc: 0.7, n: 100 } });
