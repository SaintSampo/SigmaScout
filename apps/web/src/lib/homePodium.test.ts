...
describe("pooledAccuracyPodium", () => {
  it("pools as a scoredCount-weighted mean, never a mean of means", () => {
    const a = artifact(2024, { bpr: { acc: 0.8, n: 100 }, epa: { acc: 0.7, n: 100 }, opr: { acc: 0.7, n: 100 } });
    const b = artifact(2025, { bpr: { acc: 0.6, n: 900 }, epa: { acc: 0.7, n: 900 }, opr: { acc: 0.75, n: 900 } });
    const podium = pooledAccuracyPodium([seasoned(2024, a), seasoned(2025, b)]);
    const premier = podium.find((p) => p.algorithmId === "bpr")!;
  ...
  it("sorts best-first — the podium order", () => {
    const a = artifact(2024, { bpr: { acc: 0.76, n: 10 }, epa: { acc: 0.72, n: 10 }, opr: { acc: 0.73, n: 10 } });
    expect(pooledAccuracyPodium([seasoned(2024, a)]).map((p) => p.algorithmId)).toEqual(["bpr", "opr", "epa"]);
  });

  it("a null-accuracy slice contributes nothing rather than poisoning the pool", () => {
    const a = artifact(2024, { bpr: { acc: null, n: 50 }, epa: { acc: 0.7, n: 50 }, opr: { acc: 0.7, n: 50 } });
    const b = artifact(2025, { bpr: { acc: 0.8, n: 100 }, epa: { acc: 0.7, n: 100 }, opr: { acc: 0.7, n: 100 } });
    const premier = pooledAccuracyPodium([seasoned(2024, a), seasoned(2025, b)]).find((p) => p.algorithmId === "bpr")!;
  ...
  it("throws, naming the season, when an artifact carries only OTHER seasons' slices", () => {
    const wrongSeason = artifact(2023, { bpr: { acc: 0.8, n: 100 }, epa: { acc: 0.7, n: 100 }, opr: { acc: 0.7, n: 100 } });
