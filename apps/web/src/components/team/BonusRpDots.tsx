...
import { bonusRpForSeason, type BonusRpState } from "@/lib/bonusRp";
 ...
 */
export function BonusRpDots({ season, side, states, kind, matchKey }: BonusRpDotsProps) {
      ...
      {bonuses.map((bonus, index) => {
        ...
        return (
          <span
            ...
            data-state={state}
            title={`${bonus.label}: ${state === "unknown" ? "no data published yet" : state}`}
            aria-label={`${bonus.label}: ${state === "unknown" ? "no data" : state}`}
