...
describe("EventHeader — location absence and null (E1 partial)", () => {
  ...
  it("Test 6: location null and absent location both render an em-dash, never the literal text null", () => {
    render(<EventHeader artifact={makeArtifact({ location: null })} />);
    let meta = screen.getByTestId("event-header-meta");
    expect(meta.textContent).toContain("—");
...
describe("EventHeader — startDate absence (E1 partial)", () => {
  ...
  it("Test 7: no startDate key renders an em-dash in the first segment and no invented date", () => {
    render(<EventHeader artifact={makeArtifact()} />);
    const meta = screen.getByTestId("event-header-meta");
    expect(meta.textContent?.startsWith("—")).toBe(true);
  });
});

describe("formatEventStartDate — timezone stability (PD-04)", () => {
  it("Test 8: pins UTC so the rendered day never reads as the 6th under a negative-offset TZ, and undefined returns the em-dash", () => {
...
describe("eventMetaLine — the full cross-product (E1 partial, all absence rules)", () => {
  it("Test 9: every combination of present/absent startDate, present/null/absent location, and number/zero/null/absent week returns the exact composed string", () => {
    ...
    expect(assertedCombinations).toBeGreaterThanOrEqual(12);
  });

  it("eventMetaLine({}) returns three em-dash segments", () => {
