...
function makeUpcomingMatch(overrides: Record<string, unknown> = {}): EventUpcomingMatch {
  return {
    ...
    // Quick task 260913-g66: an upcoming SPR row carries the published Match
    // Band too, under the renamed keys.
    redMatchBandVariance: 144,
    blueMatchBandVariance: 169,
    ...overrides,
...
describe("Per-tab domain (D-12)", () => {
  ...
  it("rendering ElimsTab and QualsTab against one artifact with disjoint qual/elim score ranges produces DIFFERENT tick labels", () => {
    ...
    const { unmount } = renderWithRouter(<QualsTab artifact={artifact} algorithmId="spr" season={2025} />);
    const qualsTicks = screen.getAllByTestId("axis-tick").map((t) => t.textContent);
    unmount();

    renderWithRouter(<ElimsTab artifact={artifact} algorithmId="spr" season={2025} />);
  ...
  it("two renders of the same artifact produce identical tick labels", () => {
    ...
    const { unmount } = renderWithRouter(<ElimsTab artifact={artifact} algorithmId="spr" season={2022} />);
...
describe("Bonus-RP dots — the tab's defining negative", () => {
  it("every dot on every elimination row renders data-state of unknown, across all four elimination levels", () => {
    ...
    renderWithRouter(<ElimsTab artifact={makeArtifact({ matches, season: 2024 })} algorithmId="spr" season={2024} />);
  ...
  it("a 2024 elimination row renders two dots per alliance and a 2025 one renders three, driven by bonusRpForSeason", () => {
    ...
    const { unmount } = renderWithRouter(<ElimsTab artifact={makeArtifact({ matches: matches2024, season: 2024 })} algorithmId="spr" season={2024} />);
    ...
    renderWithRouter(<ElimsTab artifact={makeArtifact({ matches: matches2025, season: 2025 })} algorithmId="spr" season={2025} />);
...
describe("Unplayed and absent-variance rows", () => {
  it("an unplayed elimination row renders both alliance bands and ticks and NO actual dot for either alliance; Actual and Call cells render an em-dash", () => {
    ...
    renderWithRouter(<ElimsTab artifact={makeArtifact({ upcoming })} algorithmId="spr" season={2022} />);
  ...
  it("a row carrying neither variance field renders both ticks and no band for either alliance and does not throw", () => {
    const matches = [makePlayedMatch({ matchKey: "qf1m1", redMatchBandVariance: undefined, blueMatchBandVariance: undefined })];
    expect(() => renderWithRouter(<ElimsTab artifact={makeArtifact({ matches })} algorithmId="spr" season={2022} />)).not.toThrow();
  ...
  // The band on this page is the published Match Band, the byte-identical
  // number the TEAM page shows for the same match.
  it("a row carrying only the red variance field renders a red band and no blue band — the published value is honoured, not overridden", () => {
    const matches = [makePlayedMatch({ matchKey: "qf1m1", redMatchBandVariance: 25, blueMatchBandVariance: undefined })];
    renderWithRouter(<ElimsTab artifact={makeArtifact({ matches })} algorithmId="spr" season={2022} />);
  ...
  it("a row carrying the NEW band keys draws both bands, played and upcoming", () => {
    ...
    for (const key of ["qf1m1", "qf1m2"]) {
      expect(screen.getByTestId(`alliance-mark-${key}-red-band`)).toBeDefined();
      expect(screen.getByTestId(`alliance-mark-${key}-blue-band`)).toBeDefined();
    }
  });

  it("IGNORES the algorithm's own redScoreVarianceOwn — reading it is what made the band a VPR privilege", () => {
    const matches = [
      makePlayedMatch({ matchKey: "qf1m1", redScoreVarianceOwn: 900, blueScoreVarianceOwn: 900, redMatchBandVariance: undefined, blueMatchBandVariance: undefined }),
    ];
    renderWithRouter(<ElimsTab artifact={makeArtifact({ matches })} algorithmId="spr" season={2022} />);
    expect(screen.getByTestId("alliance-mark-qf1m1-red-tick")).toBeDefined();
    expect(screen.queryByTestId("alliance-mark-qf1m1-red-band")).toBeNull();
...
describe("Empty state (EVNT-06 empty, UI-SPEC E6 empty)", () => {
  ...
  it("the empty-state body is byte-identical to QualsTab's exported constant, compared by import rather than retyped", () => {
    const matches = [makePlayedMatch({ matchKey: "qm1", compLevel: "qm", setNumber: 1, matchNumber: 1 })];
    renderWithRouter(<ElimsTab artifact={makeArtifact({ matches, upcoming: [] })} algorithmId="spr" season={2025} />);
    expect(screen.getByText(QUALS_EMPTY_STATE_BODY)).toBeDefined();
  });
  ...
  it("an artifact carrying no name field falls back to the event key in the heading", () => {
    const matches = [makePlayedMatch({ matchKey: "qm1", compLevel: "qm", setNumber: 1, matchNumber: 1 })];
    renderWithRouter(<ElimsTab artifact={makeArtifact({ matches, upcoming: [], name: undefined, eventKey: "2025srsd" })} algorithmId="spr" season={2025} />);
...
describe("Anti-drift against the sibling tab", () => {
  it("rendering ElimsTab and QualsTab against the same artifact, the two scroll elements' class strings are IDENTICAL", () => {
    ...
    const artifact = makeArtifact({ matches });
    const { unmount } = renderWithRouter(<QualsTab artifact={artifact} algorithmId="spr" season={2022} />);
