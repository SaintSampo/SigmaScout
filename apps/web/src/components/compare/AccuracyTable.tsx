...
/**
 * The Compare accuracy table: one uniform table, five ascending season rows
 * against three `PUBLISHED_ALGORITHM_IDS`-ordered algorithm column-groups,
 * no tune/holdout tiering anywhere. A cell renders at weight 600 ONLY where
 * `buildRowEmphasis` (backed by `compareTie.ts`'s two computed tie tests)
 * names a real leader for that cell's metric — never a naive max/min, and
 * never by adding a treatment to the "losing" cell.
 */

export const COMPARE_ACCURACY_SCROLL_TESTID = "compare-accuracy-scroll";

/** The Copywriting Contract's two exact header strings — read rather than retyped elsewhere on the page. */
 ...
 * future consumer (e.g. the near-tie rule) tell the two cases apart without
...
export interface AccuracyCell {
  ...
  /**
   * The matched slice's own `scoredCount`. Exists solely so
...
/**
 * Pure row model. Walks `COMPARE_SEASONS` in order, and for each season
 * walks `PUBLISHED_ALGORITHM_IDS` in order, selecting the single slice from
 * THAT season's own fetched artifact whose `algorithmId`/`season`/`compLevelView`
 ...
 * arithmetic on any published figure beyond this selection: the rendered
 * numbers ARE the artifact's numbers.
 */
 ...
 * is printed, so the Brier near-tie test (defined on the DISPLAYED value)
 * and the digits a reader sees can never disagree. This wrapper only adds
 * the em-dash for a null/absent figure; the underlying published figures
 * stay deliberately unrounded — this is display formatting only, never a
...
/**
 * The emphasis for one row — the two leader arrays (Brier lower-is-better,
 ...
 * `resolveWinnerAccuracyLeaders`, so the rule has exactly one implementation
 * and `compareTie.test.ts` is the test of record for it. An empty array
 * means no emphasis anywhere for that metric in that row — the near-tie
 * state, rendered as full-ink plain weight on both/all cells, never greyed.
 */
...
/**
 * The two-row grouped header: a row-label header spanning two rows, three
 * algorithm-group headers each spanning two columns, each carrying a left
 * border so the group header's span and its two body columns land on the
 * same boundary — including the first group, so no group drifts out of
 * alignment.
 */
...
export function AccuracyTable({ artifactsByYear, compLevelView }: AccuracyTableProps) {
  ...
  return (
    ...
    >
      {/* Raw <table>, NOT shadcn's <Table>: <Table> wraps its own
          [data-slot="table-container"] overflow-x-auto div, which absorbs
          all horizontal overflow and would leave this testid'd region never
          overflowing by construction. The row/cell primitives
          (TableBody/TableRow/...) stay shadcn. */}
            ...
            return (
              <TableRow key={row.season}>
                ...
                {PUBLISHED_ALGORITHM_IDS.map((algorithmId) => {
                  ...
                  // Weight ONLY, never colour — the near-tie state is
