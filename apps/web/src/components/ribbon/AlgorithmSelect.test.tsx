...
let mockSearch: Record<string, unknown> = { year: 2024, algorithm: "spr" };
...
describe("useAlgorithmOptions", () => {
      ...
      new Response(
        JSON.stringify({
          ...
          algorithms: [
            ...
            { id: "spr", version: "1.0.0+baseline", codeVersion: "1.0.0", paramSetName: "baseline" },
          ],
        }),
        { status: 200 }
      )
  ...
  // it returns "SPR" for the published id, and the manifest-merge cases
  // above append the version suffix to that SAME base label, never a
  // re-typed literal.
  it("algorithmDisplayLabel returns SPR for the published id — the single source useAlgorithmOptions' merge reads from", () => {
    expect(algorithmDisplayLabel("spr")).toBe("SPR");
...
describe("AlgorithmSelect", () => {
  ...
  afterEach(() => {
    ...
    mockSearch = { year: 2024, algorithm: "spr" };
    cleanup();
    vi.restoreAllMocks();
  });
  ...
  it("MANIFEST FAILURE: renders no error banner and the trigger/selection stay usable", async () => {
    ...
    expect(screen.getByRole("combobox", { name: "Algorithm" }).textContent).toBe("SPR");
  });
  ...
  it("selecting the already-selected value performs no navigation", async () => {
    ...
    const vprOption = await screen.findByRole("option", { name: "SPR" });
    fireEvent.pointerUp(vprOption, { button: 0, pointerId: 1 });
    fireEvent.click(vprOption);

    // Reselecting the currently-selected id ("spr", per mockSearch above)
