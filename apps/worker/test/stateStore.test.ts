...
describe("stateStore", () => {
  ...
  describe("readScopedState", () => {
    it("issues exactly one D1 statement regardless of key count (21 keys)", async () => {
      ...
      db.algorithmState.set("spr league league", {
        algorithm_id: "spr",
      ...
      for (const key of keys.slice(0, 5)) {
        db.algorithmState.set(`spr team ${key}`, {
          algorithm_id: "spr",
          algorithm_version: "2.0.0+test",
          scope_kind: "team",
          scope_key: key,
      ...
      const rows = await readScopedState(db as unknown as D1Database, "spr", [{ scopeKind: "team", scopeKeys: keys }]);
    ...
    it("throws a named error rather than issuing a second statement past MAX_SCOPE_KEYS_PER_READ", async () => {
      ...
      await expect(readScopedState(db as unknown as D1Database, "spr", [{ scopeKind: "team", scopeKeys: tooMany }])).rejects.toThrow(
        /MAX_SCOPE_KEYS_PER_READ/
      );
