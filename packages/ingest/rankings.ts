/**
 * The event-rankings normalize rule: turns TBA's `/event/{key}/rankings`
 * response into a per-team array, or an honest empty array when TBA has no
 ...
 * at all — confirmed live), and separately `rankings` can be a genuine
 * empty array (a scheduled event whose quals have not yet run — confirmed
 * live). Both are real, distinct answers, not parse failures — this
 * function treats both as "nothing to report" and returns `[]` for either,
 * never coercing one into the other and never throwing. Distinguishing a
 * null body from an empty array for logging purposes is
 * `packages/ingest/cli.ts`'s `ingestSeasonRankingsOnly`'s job, not this
 * pure function's — this module has no I/O and no corpus import, mirroring
 * `media.ts`'s shape.
 *
 * TBA's `sort_orders` array is a positional list whose VOCABULARY
 * genuinely varies by season (`sort_order_info` names — 2022 carries "Avg
 * Hangar", 2024 "Avg Coop", 2026 "Avg Tower"), but
 * `RANKING_SCORE_SORT_ORDER_INDEX` (position 0) was confirmed live to carry
 * `RANKING_SCORE_SORT_ORDER_NAME` ("Ranking Score") across every sampled
 * event and TBA event type. That finding licenses reading position 0 — it
 * does not license reading it silently. Every populated response is
 * asserted against that name before `rankingScore` is read from that
 * position; a mismatch throws `RankingScoreSortOrderError` rather than
 * storing a value read from an unasserted position. The guard and the read
 * it protects live in this one function so no future caller can perform
 * one without the other — a rule expressed as two independent literals can
 * drift apart.
 */
 ...
 * pages, confirmed live at `sort_order_info[0].name` across every sampled
 * event and TBA event type. This is the ONLY place in this module this
 * string appears as a literal — every other reference goes through this
 * constant.
 */
 ...
 * a silent hardcoded index-0 read is forbidden, and a named constant makes
...
export interface NormalizedEventRanking {
   ...
   * misreport. Required and non-nullable: `tbaEventRankingSchema`'s
   ...
   * `RANKING_SCORE_SORT_ORDER_NAME` before being read. `null` when
