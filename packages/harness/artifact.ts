/**
 * The versioned, Zod-validated canonical JSON artifact (D-02, D-03). Per
 * D-02 this JSON is canonical and the HTML report (Task 3) renders from it,
 * never the reverse — the Phase 8 Compare page reads this exact shape, so
 * it is a contract from today. `HarnessArtifactSchema` is the executable
 * specification: validating on write is the guard against the failure this
 * project has already recorded once (documentation describing a model that
 * no longer exists) — a schema that fails a test the moment the shipped
 * shape drifts cannot go stale quietly.
 */
