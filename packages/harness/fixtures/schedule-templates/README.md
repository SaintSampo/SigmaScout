...
sidecar per event, which loads a template, so those tests in
...
Skipping those tests would have retired a gate worth keeping: the presim
sidecar tests that guard the cold-publish defect fixed in `41dbbe2d`.
