 ...
 * aggregate from matches where `m.week == 1`. This corpus stores TBA's
 * week 0-indexed, so Statbotics' week 1 is this corpus's `week === 0`.
 * Getting that backwards fails silently — the model still runs and every
 * downstream assertion still passes, just calibrated on the wrong week —
 * which is why the mapping is a named, corpus-backed-tested constant
 * rather than an inline `=== 0` at the read site.
 *
 * THE NULL-WEEK POLICY: championship, preseason and offseason events all
 * carry `week = null`, a heterogeneous bucket spanning both before and
 * after week 1. A null-week match is never part of the week-1 population
 * and never triggers the week-1 freeze — it is unplaced, and treating
 * unplaced play as either would be a guess.
 *
 ...
 * numeric week greater than 0 proves every week-1 match has already
 * passed — the freeze trigger needs no lookahead, which is what makes it
 * walk-forward-legal. Before the seal, `frozen` is `null` and every caller
 * falls back to its live expanding estimate.
 *
 * ONE-MATCH LAG: the seal happens inside `update`, so the very first
 * week-2 match of a season is predicted before its own fold seals the
 * aggregate, and still reads the live estimate. Left as-is deliberately —
 * closing it would mean a second, lazily-computed read path in `predict`
 * that could drift.
 *
 * LATE WEEK-1 ARRIVALS: once sealed, this state ignores late-arriving
 * week-1 matches rather than reopening — a frozen constant that keeps
 * moving is not a constant.
 *
 ...
 * `m.week == 1`. See this module's header for the 0-indexing evidence; do
 * not inline this as a literal anywhere, and do not "fix" it to 1.
 */
export const STATBOTICS_WEEK_ONE_CORPUS_WEEK = 0;

/**
 * True when this match belongs to the population Statbotics averages its
 * `Year` aggregates over. `null` is not week 1 — see the null-week policy
...
export interface EpaWeekOneFoulAggregate {
  /** `foul_mean / no_foul_mean` — `get_foul_rate()`. */
...
export interface EpaWeekOneState {
   ...
   * `foulPoints + adjustPoints`, taken as the complement of the no-foul
   * side from the same score, so `noFoulMean + foulMean` equals the raw
   * score mean over the same folded population by construction, and
   * therefore `(1 + rate) * noFoulMean === scoreMean` exactly.
   */
 ...
 * variance to report at all. A legality floor, not a tuned threshold.
 */
export const EPA_WEEK_ONE_MIN_OBS = 2;

/**
 * Minimum week-1 foul/no-foul observations before a foul-rate freeze takes
 * effect. 1 is a mean's own contract boundary, a separate constant from
 * `EPA_WEEK_ONE_MIN_OBS` because the two gates govern different records
 * with different mathematical requirements.
 */
export const EPA_WEEK_ONE_MIN_FOUL_OBS = 1;

/**
 * The foul rate used when no usable week-1 foul information exists. 0
 * means "publish each alliance's plain no-foul total" — the identity
 * element of the `(1 + rate)` scalar, not a tuned value.
 *
 * Upstream's own guard is weaker: `get_foul_rate()` substitutes a
 * denominator of 1 and publishes `foul_mean` itself as a rate when the
 * no-foul mean is missing. This project refuses the divide instead.
 */
 ...
 *
 * Both values are dropped together when either is non-finite: they are two
 * halves of one observation of one alliance's score, and folding half of
 * it would break the `noFoulMean + foulMean === scoreMean` identity the
 * complement construction exists to guarantee.
 *
 * The caller owns the two exclusions this function cannot see — a
 * ruling-zero alliance and an alliance whose breakdown did not parse. This
 * function only knows about weeks and finiteness.
 */
 ...
 * no_foul_mean`, or `null` when the quantity is not one a prediction may
 * be multiplied by. Both the seal below and `epa.ts`'s live pre-seal read
 * call this one function, so a second copy of the ratio can never drift.
 *
 * Refused rather than fudged: a non-finite input on either side; a
 * no-foul mean that is not strictly positive; a negative foul mean, which
 * would deflate both published scores and which the sport cannot produce.
 *
 ...
 * zero SD would make the logistic scale infinite.
 *
 ...
 * population, but their gates are independent (`EPA_WEEK_ONE_MIN_OBS`
 * governs `frozen`, `EPA_WEEK_ONE_MIN_FOUL_OBS` governs `frozenFoul`), so
