...
**Live source for the site.** Both arms and the head-to-head win-probability figures are published as one object at `v1/methodology/epa-vs-statbotics.json` and rendered on `/methodology/epa-vs-statbotics`. `scripts/publishEpaComparison.ts` refuses to compose a pair whose `epaVersion` or season set disagree, so the mixed-version state this paragraph used to describe cannot be published even by accident.
