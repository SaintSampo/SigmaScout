...
**Version status, current as of 2026-09-13. SHIPPING AND PUBLISHED MODEL IS `epa@10.0.0+baseline` (quick task 260911-l2k, 2026-09-11): the foul term is now a post-win-probability scalar (§2), on top of 9.0.0's week-1 freeze (§4). THE REPUBLISH DEBT BELOW IS PAID.** Commit `ce23485e` (the Phase 9 production window, 2026-09-11) republished every season at generation `b23d214d` under 10.0.0. It also republished `v1/methodology/epa-vs-statbotics.json` from the warm arm and re-baselined `data/baselines/epa-vs-statbotics-2026-09.json` onto that arm (`docs/publish-budget.md`). The live `v1/manifest/algorithms.json` still names `epa@10.0.0+baseline` (checked 2026-09-12). The §4 page-copy debt was paid separately by quick task 260912-tib, which rewrote `/methodology/epa-vs-statbotics` from scratch (see §4).

**Previous status, retained because the paragraphs below are written against it. SHIPPING MODEL WAS `epa@9.0.0+baseline` (quick task 260911-j2w, 2026-09-11): EPA's win-probability denominator and its season-boundary carry anchor both read a FROZEN WEEK-1 aggregate from week 2 onward, and the live expanding estimate during week 1 only (§4 below). REPUBLISH DEBT IS NOW CUMULATIVE AND UNPAID: 8.0.0 already owed one and it was never run, and 9.0.0 adds a second. Every published EPA rating and every published `pRedWin` on the live site predates BOTH. `scripts/epaVsStatbotics.ts --check` was ALREADY failing against `data/baselines/epa-vs-statbotics-2026-09.json` under 8.0.0 and still fails; that is pre-existing and was deliberately NOT re-baselined by j2w.**
...
**Since quick task 260912-tib (2026-09-12) the page no longer RENDERS the agreement arm at all.** Jacob's decision: the rewritten page shows only the head-to-head winner-accuracy and Brier table. `agreement` is still composed, validated and published inside `v1/methodology/epa-vs-statbotics.json`, and the schema still requires it, but no surface on the site displays it.
...
**PAID 2026-09-12 by quick task 260912-tib (commits `80061abd`, `652e7bd3`), live and verified by content on 2026-09-13.** The page was rewritten from scratch, and the `win-probability-scale` entry and its false sentence no longer exist. The new "Season numbers from week 1" card covers both halves. It says Statbotics takes the score spread and foul rate from all of week 1. It also says SigmaScout uses those same week-1 numbers from week 2 on, with a running estimate during week 1. The paragraph below is kept as the record of the debt.
